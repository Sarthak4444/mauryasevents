...
export default function DancePartyAdminPage() {
  ...
  const correctPasscode = "dance2026";

  const fetchBookings = async () => {
    try {
      const response = await fetch("/api/dance-party/new-event");
      if (!response.ok) {
  ...
  return isAuthenticated ? (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold mb-2 text-center">Dance Party - Ticket Bookings</h1>
      <p className="text-gray-600 text-center mb-6">July 18th, 5:00 PM – 10:00 PM</p>
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full px-4 py-3 rounded-lg border-2 border-[#d88728] focus:outline-none"
        />
      </div>
    ...
    <div className="flex items-center justify-center h-screen bg-gray-100">
      <form onSubmit={handlePasscodeSubmit} className="bg-white p-6 rounded-lg shadow-lg text-center">
        <h2 className="text-2xl font-bold mb-4">Dance Party Admin</h2>
        ...
        <button
          type="submit"
          className="bg-[#d88728] text-white px-6 py-2 rounded-lg hover:bg-[#c07a24] w-full"
        >
          Submit
        </button>
      </form>
